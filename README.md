...
* **Centralized news refresh routing** — live price alerts currently refresh Bloomberg via a single `refresh_news()` hook that can be extended later to more sources
...
run.sh
  └── monsieur_market.py              <- brain, orchestrator, signal router
        ...
        │
        ├── core/
        │     └── price_state.py      <- rolling tick history + keyed price-alert cooldowns
        │
        ├── bloomberg/
...
Price spike     →  refresh_news(["bloomberg"])
                →  BloombergScheduler.trigger_now()
...
All Telegram messages originate from `monsieur_market.py` via the `telegram/` package.
...
├── run.sh                        <- start script
│
├── core/                         <- shared MM domain state
│   ├── __init__.py
│   └── price_state.py            <- rolling tick history + keyed alert cooldowns
│
...
│   ├── __init__.py               <- exports: get_ig_service, start_price_watcher, etc.
...
│
├── telegram/                     <- Telegram messaging + command receiver
│   └── ...
│
├── trumpstruth/                  <- Trump watcher
│   └── ...
│
...
Live price alerts do not directly hardcode Bloomberg refresh anymore.

Instead, MM routes refresh requests through a central `refresh_news()` function.

Current live configuration:

* **price alerts** → `refresh_news(["bloomberg"])`

This keeps the live path simple today while making it easy to extend later with additional sources such as:
...
| `rss_at_open`        | no       | Reserved / future use      |
...
* [ ] Extend `refresh_news()` to more sources
